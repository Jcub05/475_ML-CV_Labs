...
def parse_results_txt(txt_path):
    ...
    # Extract model type and KD mode
    model_match = re.search(r'Model:\s*(\w+)', content)
    kd_match = re.search(r'KD Mode:\s*(\w+)', content)
    params_match = re.search(r'Parameters:\s*([\d,]+)', content)
    
    if model_match:
        result['model_type'] = model_match.group(1)
    if kd_match:
        result['kd_mode'] = kd_match.group(1)
    if params_match:
        params_str = params_match.group(1).replace(',', '')
        result['num_params'] = int(params_str)
    
    # Extract metrics
    miou_match = re.search(r'Mean IoU:\s*([\d.]+)', content)
    inference_match = re.search(r'Inference Time:\s*([\d.]+)\s*ms', content)
    score_match = re.search(r'Score:\s*([\d.]+)', content)
    
    if miou_match:
        result['mean_iou'] = float(miou_match.group(1))
    if inference_match:
        result['avg_inference_time'] = float(inference_match.group(1))
    if score_match:
        result['score'] = float(score_match.group(1))
    
    # Calculate FPS
    if 'avg_inference_time' in result and result['avg_inference_time'] > 0:
    ...
    for idx, cls_name in enumerate(voc_classes):
        # Match class name followed by IoU value
        pattern = rf'{re.escape(cls_name)}\s*:\s*([\d.]+)'
        match = re.search(pattern, content)
        if match:
            result['class_ious'][idx] = float(match.group(1))
    
    return result


def load_results(results_dir='./Results'):
    """Load all results .txt files"""
    ...
    for model in models:
        for kd_mode in kd_modes:
            # Check in subdirectory structure
            txt_path = results_dir / model / f'{model}_{kd_mode}' / f'results_{model}_{kd_mode}.txt'
            if txt_path.exists():
...
def load_training_info(checkpoints_dir='./Results'):
    ...
    for model in models:
        for kd_mode in kd_modes:
            # Check in subdirectory structure
            ckpt_path = checkpoints_dir / model / f'{model}_{kd_mode}' / f'best_model_{model}_{kd_mode}.pth'
            if ckpt_path.exists():
                try:
                    ...
                    print(f"Warning: Could not load training info from {ckpt_path}: {e}")
...
def main():
    ...
    # Save text version
    text_output = Path('./comparison_table.txt')
    headers, table_data = generate_table(results, training_info)
