...
# ============================================================================
# These aliases ensure existing scripts (train.py, test.py, etc.) continue to work

# Alias for CLIPImageEncoder (baseline version without modifications)
CLIPImageEncoder = lambda embed_dim=512, pretrained=True, freeze_backbone=False: CLIPImageEncoderModified(
    embed_dim=embed_dim,
    use_batchnorm=False,
    use_dropout=False,
    deeper_projection=False,
...
# Alias for CLIPFineTuneModel (uses the modified architecture with baseline config)
CLIPFineTuneModel = CLIPFineTuneModelModified
...
def create_clip_model(text_encoder, tokenizer, embed_dim=512):
    ...
    Used by ablation_study.py and other scripts for compatibility.
    ...
    Returns:
        CLIPFineTuneModelModified instance with baseline config
    """
    # Create baseline model (all modifications disabled)
